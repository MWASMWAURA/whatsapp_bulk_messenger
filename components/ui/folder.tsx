...
interface FolderProps {
  ...
  className?: string;
  children: ReactNode;
  /** Optional callback when files are selected via the folder upload */
...
const Folder: React.FC<FolderProps> = ({
  ...
  const handleClick = () => {
    const next = !isOpen;
    setIsOpen(next);
    // If opening, trigger file picker so user can upload while folder opens
    if (next) {
  ...
  const handleFiles = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    else console.log("Selected files:", files);
    ...
    <FolderContext.Provider value={{ isOpen, size }}>
      <div className={cn("w-full max-w-2xl flex items-center justify-center", className)}>
        <Tooltip>
          ...
          <TooltipContent sideOffset={6} className="z-50">
            <div className="text-sm">Add contact</div>
          </TooltipContent>
        </Tooltip>

        {/* hidden file input to trigger uploads when folder is opened */}
        <input ref={fileInputRef} type="file" className="hidden" multiple accept=".csv,.xlsx,.xls" onChange={handleFiles} />
      </div>
